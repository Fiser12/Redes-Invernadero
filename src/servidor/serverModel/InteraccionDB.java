...
public class InteraccionDB {
	...
	public static void crearBaseDeDatos()
	{
		...
		gestor.enviarComando("CREATE TABLE IF NOT EXISTS Sensor (Id INTEGER PRIMARY KEY AUTOINCREMENT, Nombre_Variable VARCHAR(250) NOT NULL, Estado_la_variable VARCHAR(250) NOT NULL,Ultima_Accion VARCHAR(500) NOT NULL, id_Placa INT(10), CONSTRAINT id_Placa FOREIGN KEY (id_Placa) REFERENCES Placa (Id) ON DELETE CASCADE ON UPDATE CASCADE, CONSTRAINT Nombre_Variable FOREIGN KEY (Nombre_Variable) REFERENCES Variable(Nombre) ON DELETE CASCADE ON UPDATE CASCADE);");
	}
